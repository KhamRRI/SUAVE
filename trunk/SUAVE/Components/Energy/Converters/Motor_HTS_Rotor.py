...
class Motor_HTS_Rotor(Energy_Component):
    ...
    The power used here could be considered the same as that used by the motor as inefficiency, however many
    publications consider the overall motor efficiency, i.e. including cryocooler and/or motor drive electronics.
    
    Assumptions:
    No ACLoss in the HTS,
    HTS is operated within the Ic and Tc limits.
    i.e the power used by the coil is only due to solder resistances only, and is not affected by the motor output
    power or speed.
    ...
    def power(self, current, ambient_temp):
        """ Calculates the electrical power draw from the HTS coils, and the total heating load on the HTS rotor
        cryostat.
        ...
        coil_resistance  = self.resistance
        surface_area     = self.surface_area
        r_value          = self.r_value

        # Calculate HTS coil power
        # This is both the electrical power required to operate the coil, and the thermal load imparted by the coil
        # into the cryostat.
        coil_power       = coil_resistance * current**2
        coil_voltage     = current*coil_resistance

        # Estimate heating from external heat conducting through the cryostat wall.
        # Given the non-cryogenic armature coils are usually very close to this external wall it is likely the
        # temperature at the wall is above ambient.
